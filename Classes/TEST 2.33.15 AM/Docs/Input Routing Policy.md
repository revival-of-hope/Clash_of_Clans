...
## Contract
* `Engine::InputRouter` (Contracts path: `Classes/Contracts/Engine/InputRouter.h`).
```
...
## Semantics
* **UI-first:** If `consume` is true, all routes return `false` and no map/highlighter updates occur.
