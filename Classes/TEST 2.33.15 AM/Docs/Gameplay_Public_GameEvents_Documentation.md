...
Module: Classes/Contracts/Gameplay/GameEvents.h
...
### **3.2 Entity Spawn (EntitySpawnEvent)**
...
### **3.3 Entity Destroy (EntityDestroyEvent)**
...
### **3.4 Building State Change (BuildingStateEvent)**
...
**Access pattern:** The latest `BattleEndEvent` is cached in `GameEventManager` (Contracts path `Classes/Contracts/Gameplay/GameEvents.h`) and can be retrieved via `GetLastBattleEnded()` even if no listener was registered in time.
...
```cpp
#include "Classes/Contracts/Gameplay/GameEvents.h"
